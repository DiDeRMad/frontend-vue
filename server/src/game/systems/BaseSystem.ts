...
export abstract class BaseSystem extends EventEmitter {
  protected world!: GameWorld;
  protected name: string;
  protected isInitialized: boolean = false;
  protected updateInterval: number = 0; // 0 means update every tick
  ...
  async initialize(): Promise<void> {
    if (this.isInitialized) {
      logger.warn(`System ${this.name} already initialized`);
      return;
    }

    logger.info(`Initializing system: ${this.name}`);
  ...
  async shutdown(): Promise<void> {
    ...
    logger.info(`Shutting down system: ${this.name}`);
  ...
  async update(deltaTime: number): Promise<void> {
    ...
    // Check if should update based on interval
  ...
  protected getUpdateInterval(): number {
    return 0; // Override to set custom update interval
  }
